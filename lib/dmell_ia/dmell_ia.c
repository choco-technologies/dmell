...
/**
 * @brief Interactive-mode state: command history.
 *
...
typedef struct
{
    char*  history[DMELL_HISTORY_MAX];
    size_t history_count;
    size_t history_head;
} dmell_ia_state_t;

/**
 * @brief Returns ctx's interactive-mode state, allocating and zeroing it on first use.
 *
 * @param ctx Per-session context
 * @return dmell_ia_state_t* State, or NULL on allocation failure
 */
static dmell_ia_state_t* get_ia_state( dmell_ctx_t* ctx )
{
    if( ctx->ia == NULL )
    {
        dmell_ia_state_t* state = Dmod_Malloc( sizeof(dmell_ia_state_t) );
        if( state != NULL )
        {
    ...
    return (dmell_ia_state_t*)ctx->ia;
}
 ...
 *
 * @param state Interactive-mode state
 * @param line The command line to store
 */
static void history_add(dmell_ia_state_t* state, const char* line)
{
    ...
    if( state->history[state->history_head] != NULL )
    {
        Dmod_Free(state->history[state->history_head]);
        state->history[state->history_head] = NULL;
    }

    size_t len = strlen(line);
    state->history[state->history_head] = Dmod_Malloc(len + 1);
    if( state->history[state->history_head] != NULL )
    {
        memcpy(state->history[state->history_head], line, len + 1);
    }

    state->history_head = (state->history_head + 1) % DMELL_HISTORY_MAX;
    if( state->history_count < DMELL_HISTORY_MAX )
    {
        state->history_count++;
    }
}
 ...
 */
static const char* history_get(dmell_ia_state_t* state, size_t offset)
{
    if( offset >= state->history_count )
    {
        return NULL;
    }
    size_t idx = (state->history_head + DMELL_HISTORY_MAX - 1 - offset) % DMELL_HISTORY_MAX;
...
static bool find_builtin_command_match(const char* partial_name, char* out_match, size_t max_length)
{
    ...
    return dmell_find_command_prefix_match(partial_name, out_match, max_length);
}
 ...
 *
 * @param state Interactive-mode state (history for arrow-key navigation)
 * @param out_len Output parameter to hold the length of the read line
 * @return char* The read line, or NULL on failure
 */
static char* read_line( dmell_ia_state_t* state, size_t* out_len )
{
    print_prompt();
        ...
        else if( c == 27 ) // ESC - potential arrow key sequence
        {
            ...
            if( c2 == '[' )
            {
                ...
                if( c3 == 'A' ) // Arrow up – navigate to an older history entry
                {
                    ...
                    const char* hist = history_get(state, new_history_pos - 1);
                    if( hist != NULL )
                    {
                ...
                else if( c3 == 'B' ) // Arrow down – navigate to a newer history entry
                {
                    if( history_pos > 0 )
                    {
                        ...
                        {
                            new_content = history_get(state, history_pos - 1);
...
int dmell_interactive_mode( dmell_ctx_t* ctx )
{
    dmell_ia_state_t* state = get_ia_state( ctx );
    if( state == NULL )
    {
        DMOD_LOG_ERROR("Memory allocation failed for interactive mode state\n");
        return -ENOMEM;
    }
    ...
    while( true )
    {
        ...
        dmell_run_script_line(ctx, line, line_len );
