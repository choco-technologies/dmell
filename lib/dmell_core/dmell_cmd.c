 ...
 * and are threaded through explicitly instead.
 */
...
/**
 * @brief Finds the first registered command whose name starts with partial_name.
 *
 ...
 */
bool dmell_find_command_prefix_match(const char* partial_name, char* out_match, size_t max_length)
{
 ...
 *
 * @param ctx Per-session context, forwarded to the command's handler
 * @param cmd_name Name of the command to run
 * @param argc Number of arguments
 * @param argv Array of arguments
 * @return int Exit code of the command
 */
int dmell_run_command(dmell_ctx_t* ctx, const char* cmd_name, int argc, char** argv)
{
    if( cmd_name == NULL )
    {
    ...
    if( command != NULL )
    {
        return command->handler( argc, argv, ctx );
    }
    else if( g_default_command_handler != NULL )
    {
        return g_default_command_handler( argc, argv, ctx );
    }
    else
    {
 ...
 * @return int Exit code of the command
 */
int dmell_run_command_string(dmell_ctx_t* ctx, const char* cmd, size_t len)
{
    if( cmd == NULL || len == 0 )
    {
    ...
    result = dmell_run_command( ctx, command_name, parsed_argv.argc, parsed_argv.argv );
