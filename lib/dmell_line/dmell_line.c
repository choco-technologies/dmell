 ...
 *
 * @param ctx Per-session context, forwarded to command dispatch and background jobs
 ...
 */
int dmell_run_line(dmell_ctx_t* ctx, const char* line, size_t len)
{
    if(line == NULL || len == 0)
    {
 ...
 * @return int Exit code of the last executed command, or negative value on error
 */
int dmell_run_args_line(dmell_ctx_t* ctx, int argc, char** argv)
{
    if( argc <= 0 || argv == NULL )
    {
